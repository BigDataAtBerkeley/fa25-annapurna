...
from error_db import save_error, get_errors, get_latest_error, clear_errors, get_error_count
...
def extract_errors_from_result(exec_result: Dict[str, Any]) -> List[str]:
    ...
    if exec_result.get('timeout') or exec_result.get('error_type') == 'timeout':
        errors.append(f"EXECUTION ERROR: Execution timed out - {exec_result.get('error_message', 'Timeout')}")
        return errors
    ...
    if stderr:
        ...
        import re
        traceback_pattern = r'Traceback.*?((?:AttributeError|TypeError|ValueError|RuntimeError|ImportError|KeyError|IndexError|NameError|ModuleNotFoundError):\s*.+?)(?=\n\n|$)'
        matches = re.findall(traceback_pattern, stderr, re.DOTALL)
        for match in matches:
            if isinstance(match, tuple):
                error_line = match[0].strip()
            else:
                error_line = match.strip()
            if error_line:
                errors.append(f"EXECUTION ERROR: {error_line}")
        
        # If no traceback, look for error patterns
        if not errors:
            error_pattern = r'(AttributeError|TypeError|ValueError|RuntimeError|ImportError|KeyError|IndexError|NameError|ModuleNotFoundError):\s*(.+?)(?=\n|$)'
            matches = re.findall(error_pattern, stderr, re.IGNORECASE | re.MULTILINE)
            for match in matches:
                if isinstance(match, tuple):
                    error_type, error_msg = match
                    errors.append(f"EXECUTION ERROR: {error_type}: {error_msg.strip()[:200]}")
                else:
                    errors.append(f"EXECUTION ERROR: {match.strip()[:200]}")
    
    # Extract from error_message
    if exec_result.get('error_message') and exec_result.get('error_message') not in str(errors):
        errors.append(f"EXECUTION ERROR: {exec_result.get('error_message')}")
    
    # Fallback
    if not errors:
        errors.append(f"EXECUTION ERROR: Code execution failed (return code: {exec_result.get('return_code', -1)})")
    
    return errors[:5]  # Limit to 5 errors


def fix_code_with_bedrock(code: str, errors: List[str], iteration: int) -> Optional[str]:
    """
    Use Bedrock to fix code based on errors.
...
def code_review():
    ...
    try:
        ...
        if not errors_list:
            return jsonify({
                "success": False,
                "error": f"No errors found in database for {paper_id}"
            }), 404
        
        latest_error = errors_list[-1].get('error_data', {})
        errors = extract_errors_from_result(latest_error)
        ...
        logger.info(f"Found {len(errors)} errors for {paper_id}")
        ...
        error_count = get_error_count(paper_id)
        if error_count >= MAX_REVIEW_ITERATIONS:
            logger.warning(f"Max review iterations reached for {paper_id} ({error_count})")
            return jsonify({
                "success": False,
