...
This version (BedrockClient2) supports larger paper content input to better utilize Claude 3's 200k context window.
"""
...
class BedrockClient2:
    """Client for interacting with AWS Bedrock Claude models with extended paper content support."""
    ...
    def __init__(self):
        ...
        # Extended paper content limit: 200k characters (~50k tokens) to better utilize Claude 3's 200k context window
        # Accounts for large prompt template (~30k tokens), summary/recommendations (~3k tokens), and output (8k tokens)
        # Leaves ~50k tokens for paper content, which is still more than the original 150k chars (~40k tokens)
        # Original bedrock_client.py used 150k chars (~40k tokens)
        # Will automatically retry with smaller limits if input is too long
        self.max_paper_content_chars = 200000
        
        logger.info(f"Bedrock client initialized with model: {self.model_id} (timeout: 150s, max paper content: {self.max_paper_content_chars:,} chars)")
    ...
    def generate_pytorch_code(self, paper_summary: str, paper_content: Optional[str] = None,
        ...
        """
        try:
            # Try with progressively smaller paper content limits if input is too long
            content_limits = [self.max_paper_content_chars, 150000, 100000, 50000]
            
            for content_limit in content_limits:
                try:
                    # Prepare the prompt for Claude with current content limit
                    prompt = self._create_pytorch_prompt(paper_summary, paper_content, dataset_recommendations, content_limit)
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            # Check if input is too long - retry with smaller content limit
                            is_input_too_long = (error_code == 'ValidationException' and 
                                                ('too long' in error_msg.lower() or 
                                                 'input is too long' in error_msg.lower()))
                            
                            if is_input_too_long:
                                # Try next smaller content limit
                                if content_limit == content_limits[-1]:
                                    # Already at smallest limit, give up
                                    raise
                                else:
                                    logger.warning(f"Input too long with {content_limit:,} chars, retrying with smaller limit...")
                                    break  # Break out of retry loop to try next content limit
                        ...
                        # If we exhausted retries without success, continue to next content limit
                        continue
                    ...
                    if is_input_too_long and content_limit != content_limits[-1]:
                        # Try next smaller limit
                        continue
                    else:
                        # Not an input-too-long error, or already at smallest limit - re-raise
                        raise
            
            # If we get here, all content limits failed
            raise Exception("All content limit attempts failed")
    ...
    def _create_pytorch_prompt(self, paper_summary: str, paper_content: Optional[str] = None,
                               ...
                               content_limit: Optional[int] = None) -> str:
        """
        ...
        Args:
            ...
            paper_content: Full paper content (optional)
            dataset_recommendations: Optional dataset recommendations dictionary
            content_limit: Optional limit for paper content (defaults to self.max_paper_content_chars)
            
        Returns:
            Formatted prompt string
        """
        # Use provided limit or default
        limit = content_limit if content_limit is not None else self.max_paper_content_chars
...
"""
        ...
        if paper_content:
            # Use specified limit to better utilize Claude 3's 200k context window
            base_prompt += f"""
Full Paper Content:
{paper_content[:limit]}
...
{paper_content[:content_limit] if paper_content else ""}

Please provide an improved implementation addressing the feedback.
"""
                else:
                    prompt = self._create_pytorch_prompt(paper_summary, paper_content, None, content_limit)
                ...
                try:
                    ...
                    if is_input_too_long:
                        ...
                        if content_limit == content_limits[-1]:
                            ...
                            logger.warning(f"Input too long with {content_limit:,} chars, retrying with smaller limit...")
                            continue  # Try next content limit
                    else:
                        raise  # Re-raise other errors
                
                # Successfully got response, break out of content limit loop
                break
                ...
                # For non-ClientError exceptions, re-raise immediately
                raise
            
            # Parsing response (only reached if we successfully got a response)
            response_body = json.loads(response['body'].read())
            generated_text = response_body['content'][0]['text']
        ...
        # If we get here, we've exhausted all content limits
        logger.error("Error generating code with feedback: Input too long even with smallest limit")
        return {
            "success": False,
            "error": "Input too long for requested model even after reducing content limit",
            "code": None,
            "explanation": None
        }
