...
def save_to_local(paper_id: str, code: str, code_result: Dict[str, Any], output_dir: str = "generated_code") -> Optional[str]:
    """Save generated code and metadata to local filesystem (when not running in Lambda)"""
    # Check if we're running in Lambda
    if os.getenv('LAMBDA_TASK_ROOT'):
        return None  # Skip local saving in Lambda
    ...
    try:
        ...
        paper_title = code_result.get("paper_title", "unknown_paper")
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            authors = code_result.get("paper_authors", [])
            if isinstance(authors, list):
                f.write(f'Authors: {", ".join(authors)}\n')
            else:
                f.write(f'Authors: {authors}\n')
            f.write(f'Generated at: {code_result.get("generated_at", "N/A")}\n')
            f.write(f'"""\n\n')
            f.write(code)
        ...
        metadata_filepath = filepath.replace('.py', '_metadata.json')
        with open(metadata_filepath, 'w', encoding='utf-8') as f:
            json.dump(metadata, f, indent=2)
        
        logger.info(f"Saved code locally to {filepath}")
        return filepath
        
    except Exception as e:
        logger.warning(f"Failed to save code locally: {e}")
        return None
...
def save_to_s3(paper_id: str, code: str, code_result: Dict[str, Any]) -> Dict[str, str]:
    ...
    metadata = {
        ...
        "recommended_dataset": code_result.get("recommended_dataset"),
        "dataset_recommendations": code_result.get("dataset_recommendations"),
        "code_review": code_result.get("code_review")  # Include code review metadata (fixes_applied, iterations, etc.)
    }
    ...
    # Also save locally if not in Lambda
    save_to_local(paper_id, code, code_result)
...
class CodeGenHandler:
    ...
    def handle_lambda_event(self, event: Dict[str, Any], use_aws: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if use_aws and result.get('success') and result.get('code') and action != 'get_paper_info':
                result = self._handle_aws_integrations(result)
            elif use_aws and not result.get('success'):
                ...
                try:
                    ...
                    try:
                        ...
                        def generate_code_with_timeout():
                            ...
                            return handler.handle_lambda_event(message, use_aws=True)
